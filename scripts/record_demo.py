...
def record_episode(agent, game, seed=0, render_size=(420, 320), max_steps=3000,
    ...
    Trả về: gif_frames (subsample nhẹ), total_reward, n_steps
    """
    np.random.seed(seed)  # fix reproducibility cho NoopResetEnv
    ...
    gif_frames = []  # chỉ giữ subsample nhẹ cho GIF, tránh OOM
    ...
    while not done and step < max_steps:
        ...
        if raw is not None:
            ...
            if writer is not None:
                writer.append_data(raw)  # ghi ngay ra disk, không giữ RAM
            if step % 5 == 0:
